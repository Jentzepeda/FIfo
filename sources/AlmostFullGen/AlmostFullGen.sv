...
GrayToBin #(.SIZE(SIZE)) read_bin (.gray_in(r_gray_pointer),
	.bin_out(r_bin_pointer));

//gets binary values n-1 and converts sets almost flag 
AlmostFull #(.SIZE(SIZE)) a_ful(.w_pointer(w_bin_pointer[SIZE-1:0]),
	.r_pointer(r_bin_pointer[SIZE-1:0]),.a_flag(a_flag)); 
